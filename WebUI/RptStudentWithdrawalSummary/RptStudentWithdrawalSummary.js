local.RptStudentWithdrawalSummaryPageController = function (idPrefix) {
    var self = this;
    ...
    function Initialize() {
        ...
        $(btnExport).attr('disabled', false).unbind().click(function () {
            ...
            RptStudentWithdrawalSummaryAjaxGateway.GetResult(yearfrom, yearto, tsp, sponsor, true, function (res) {
                if (res.error == null) {
                    window.open("ExportReport.aspx?ReportName=RptStudentWithdrawalSummary");
                }
            });
        });
    }
    ...
    function Activated() {
        if (!local.ValidatePermission(FlagCodes.PermissionType.ReportStudentWithdrawalSummary)) {
        ...
        navigationTrail = {
            Name: "Report - Student Withdrawal Summary",
            Link: "#"
        };
        navigationTrails.push(navigationTrail);
    ...
    function BuildSurfaceListing() {
        ...
        self.ListControlller.SearchDelegate = function () {
            ...
            RptStudentWithdrawalSummaryAjaxGateway.GetResult(yearfrom, yearto, tsp, sponsor, false, function (res) {
                if (res.error == null) {
        ...
        self.ListControlller.PopulateDataDelegate = function (tbl) {
            ...
            //Generate Chart
            RptStudentWithdrawalSummaryAjaxGateway.GetColumnChart(function (res) {
                if (res.error == null) {
                    imgURL = res.value;
                    $(divChart).empty();
    ...
    var Title = "Student Withdrawal Summary Report";
    self.Title = Title;
