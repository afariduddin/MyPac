local.RptStudentProgressSummaryPageController = function (idPrefix) {
    var self = this;
    ...
    function Initialize() {
        ...
        $(btnExport).attr('disabled', false).unbind().click(function () {
            ...
            RptStudentProgressSummaryAjaxGateway.GetResult(intakefrom, intakeTo, fullname, gender, contractType, sponsorshipType,
                false, order.Columns, order.Directions, pg, function (res) {
                    if (res.error == null) {
                        window.open("ExportReport.aspx?ReportName=RptStudentProgressSummary");
                    }
                });
        });
    }
    ...
    function Activated() {
        ...
        navigationTrail = {
            Name: "Report - Student Progress Summary",
            Link: "#"
        };
        navigationTrails.push(navigationTrail);
    ...
    function BuildSurfaceListing() {
        ...
        self.ListControlller.PopulateHeaderDelegate = function (tbl) {
            ...
            for (var i = 0; i < tbl.Columns.length; i++) {
                var col = tbl.Columns[i];
                if (col.Name.indexOf("-") > 0) {
                    var colName = col.Name.substring(col.Name.indexOf("-")+1, col.Name.length)
        ...
        self.ListControlller.PopulateDataDelegate = function (tbl) {
            ...
            for (var i = 0; i < tbl.Rows.length; i++) {
                ...
                for (var j = 0; j < tbl.Columns.length; j++) {
                    ...
                    //if (col.Name.indexOf("Date") > 0) colValue = teq.Common.FormatDate(new Date(dr[col.Name]));
                    //if (col.Name.indexOf("Date") > 0) colValue = teq.Common.FormatDate(colValue);
                    //if (col.Name.indexOf("Result") > 0) colValue = Math.round(colValue);

                    tr.append(("<td align=center style='vertical-align: top'>" + colValue + "</td>"));
                }
