local.RptStudentIntakeSummaryPageController = function (idPrefix) {
    var self = this;
    ...
    var chkSearchStatus = "#" + idPrefix + "_chkSearchStatus";
    ...
    function Initialize() {
        ...
        $(btnExport).attr('disabled', false).unbind().click(function () {
            ...
            var sponsors = $(chkSearchSponsor).checkboxList("getValues");
            var statuses = $(chkSearchStatus).checkboxList("getValues");

            RptStudentIntakeSummaryAjaxGateway.GetResult(yearfrom, yearto, tsp, course,sponsors, statuses, false, order.Columns, order.Directions, pg, function (res) {
                if (res.error == null) {
                    window.open("ExportReport.aspx?ReportName=RptStudentIntakeSummary");
                }
            });
        });
    }
    ...
    function Activated() {
        if (!local.ValidatePermission(FlagCodes.PermissionType.ReportStudentIntakeSummary)) {
        ...
        navigationTrail = {
            Name: "Report - Student Intake Summary",
            Link: "#"
        };
        navigationTrails.push(navigationTrail);
        ...
        $(chkSearchStatus).checkboxList(
        {
            columnSpacing: 20,
            valuesChanged: null,
            choices: Enums.StudentStatusType, //Flags.TradeType,
            //showLinks: true,
            columns: 1
        }
        );
        ...
        self.ListControlller.PopulateDataDelegate = function (tbl) {
            ...
            for (var i = 0; i < tbl.Rows.length; i++) {
                ...
                for (var j = 0; j < tbl.Columns.length; j++) {
                    ...
                    if (col.Name != "Intake" || col.Name != "TSP") {
                ...
                if (i == tbl.Rows.length - 1) { //Run Once for Footer
                    var tr = $("<tr></tr>");
                    tr.append("<td colspan=2>Total</td>");
                    for (var c = 1; c < grandTotal.length; c++) {
            ...
            //Generate Chart
            RptStudentIntakeSummaryAjaxGateway.GetColumnChart(function (res) {
                if (res.error == null) {
                    imgURL = res.value;
                    $(divChart).empty();
