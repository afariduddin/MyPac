...
public partial class RptActiveStudentSummary : PageUserControlBase
{
    protected void Page_Load(object sender, EventArgs e)
    {
        Utility.RegisterTypeForAjax(typeof(RptActiveStudentSummaryAjaxGateway));
        AjaxLib.RegisterController("RptActiveStudentSummary", ClientID);
    }
}

public class RptActiveStudentSummaryAjaxGateway : AjaxGatewayBase
{
    [AjaxMethod]
    public DataTable GetResult(int yearfrom, int yearto, string[] tsp, string[] sponsor, bool isExport)
    {
        ...
        DataTable content = da.Reports.SearchActiveStudentSummary(yearfrom, yearto, tspJoin, sponsorJoin);
        ...
        foreach (DataRow drData in dt.Rows)
        {
            foreach (DataRow drContent in content.Rows)
            {
                ...
                {
                    drData[drContent["Course_Code"].ToString()] = drContent["ActiveCount"].ToString();
                }
            }
        ...
        if (!isExport) HttpContext.Current.Session["RptActiveStudentSummary"] = dt;
    ...
    {
        if (HttpContext.Current.Session["RptActiveStudentSummary"] == null) return "";

        DataTable dt = (DataTable)HttpContext.Current.Session["RptActiveStudentSummary"];
        ...
        Chart1.ChartAreas["ChartArea1"].AxisY.Title = "Total Active";
