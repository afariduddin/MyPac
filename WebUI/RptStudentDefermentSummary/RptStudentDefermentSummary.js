local.RptStudentDefermentSummaryPageController = function (idPrefix) {
    var self = this;
    ...
    function Initialize() {
        ...
        $(btnExport).attr('disabled', false).unbind().click(function () {
            ...
            RptStudentDefermentSummaryAjaxGateway.GetResult(yearfrom, yearto, tsp, course, true, function (res) {
                if (res.error == null) {
                    window.open("ExportReport.aspx?ReportName=RptStudentDefermentSummary");
                }
            });
        });
    }
    ...
    function Activated() {
        if (!local.ValidatePermission(FlagCodes.PermissionType.ReportDefermentSummary)) {
        ...
        navigationTrail = {
            Name: "Report - Student Deferment Summary",
            Link: "#"
        };
        navigationTrails.push(navigationTrail);
    ...
    function BuildSurfaceListing() {
        ...
        self.ListControlller.SearchDelegate = function () {
            ...
            RptStudentDefermentSummaryAjaxGateway.GetResult(yearfrom, yearto, tsp, course, false, function (res) {
                if (res.error == null) {
        ...
        self.ListControlller.PopulateDataDelegate = function (tbl) {
            ...
            //Generate Chart
            RptStudentDefermentSummaryAjaxGateway.GetPieChart(function (res) {
                if (res.error == null) {
                    imgURL = res.value;
                    $(divChart).empty();
    ...
    var Title = "Student Deferment Summary Report";
    self.Title = Title;
